...
def start_client(server_host=VOIP_SERVER, server_port=50007):
    ...
    # Output
    stream_out = audio.open(format=FORMAT, channels=CHANNELS, rate=RATE, output=True, output_device_index=DEVICE_INDEX, frames_per_buffer=CHUNK)

    # Input
    stream_in = audio.open(format=FORMAT, channels=CHANNELS, rate=RATE, input=True, input_device_index=DEVICE_INDEX, frames_per_buffer=CHUNK)
